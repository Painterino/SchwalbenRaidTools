...
rcButtonText:SetText("Ich bin bereit!");
rcButtonText:SetPoint("CENTER");
local font = select(1, rcButtonText:GetFont());
rcButtonText:SetFont(font, 13);

local rcButton = CreateFrame("Button", "SchwalbenTools_ReadyCheckButton", fB, "UIGoldBorderButtonTemplate");
rcButton:SetPoint("CENTER");
rcButton:SetSize(180,45);
rcButton:SetText("Ready!");
...
rcButton:SetScript("OnClick", function(self)
	SendChatMessage("Schwalbe: Ready!", "RAID");
	rcStatus = true;
	fB:Hide();
	ag:Stop();
	rcButton:Hide();
...
local rcCloseButton = CreateFrame("Button", "SchwalbenTools_ReadyCheckButton", f, "UIPanelButtonTemplate");
...
f:SetScript("OnEvent", function(self, event, ...)
	if (event == "READY_CHECK_CONFIRM" and SRT_ReadyCheckEnabled) then
		local id, response = ...;
		local player = UnitName("player");
		local playerIndex = SRT_GetRaidMemberIndex(player);
		--Sender part
		if ((rcSender == UnitName("player") or SRT_ReadyCheckWatcher) and select(2,GetInstanceInfo()) == "raid" and UnitIsVisible(id)) then
			...
			if (not response) then
				...
				if (rcText:GetText() == nil) then
					rcText:SetText("|cFF00FFFFSchwalbenTools|r - Players not ready or afk: \n" .. playerText .. '\n');
	...
	elseif (event == "CHAT_MSG_RAID" or event == "CHAT_MSG_RAID_LEADER") and SRT_ReadyCheckEnabled and f:IsShown() then
		if (rcSender == UnitName("player") or SRT_ReadyCheckWatcher) then
			local msg, sender = ...;
			sender = Ambiguate(sender, "short");
			if (msg == "SchwalbenTools: Ready!" and rcText:GetText():match(sender)) then
				...
				if (currentText == "|cFF00FFFFSchwalbenTools|r - Players not ready or afk: \n") then
					rcText:SetText("");
					rcText:Hide();
					f:Hide();
					rcCloseButton:Hide();
	...
	elseif (event == "READY_CHECK" and SRT_ReadyCheckEnabled) then
		local sender = ...;
		rcStatus = false;
		rcSender = sender;
	...
	elseif (event == "READY_CHECK_FINISHED" and SRT_ReadyCheckEnabled) then
		if (not rcStatus and not fB:IsShown() and select(2,GetInstanceInfo()) == "raid") then
			fB:Show();
			rcButton:Show();
		end
		if ((rcSender == UnitName("player") or SRT_ReadyCheckWatcher) and select(2, GetInstanceInfo()) == "raid") then
	...
	elseif (event == "PLAYER_LOGIN") then
		if (SRT_ReadyCheckEnabled == nil) then SRT_ReadyCheckEnabled = true; end
		if (SRT_ReadyCheckFlashing == nil) then SRT_ReadyCheckFlashing = false; end
		if (SRT_ReadyCheckWatcher == nil) then SRT_ReadyCheckWatcher = false; end
	end
end)

function SRT_GetRaidMemberIndex(name)
	for i = 1, GetNumGroupMembers() do
		local raider = "raid"..i;
...
function SRT_ReadyCheck_Test()
	rcButton:Show();
	f:Show();
	rcCloseButton:Show();
	rcText:Show();
	rcText:SetText("|cFF00FFFFSchwalbenTools|r - Players not ready: \nMalv\nAes");
	f:SetBackdropColor(0,0,0,1);
	f:SetBackdropBorderColor(169,169,169,1);
	fB:Show();
end